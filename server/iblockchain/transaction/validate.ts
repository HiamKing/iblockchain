...
const validateTransaction = (transaction: Transaction, unspentTxOuts: UnspentTxOut[]): boolean => {
  ...
  if(!hasValidTxIns) {
    console.log('Some of the txIns are invalid in transaction: ' + transaction.id);
    return false;
  }
  ...
  if(totalTxOutValues !== totalTxInValues) {
    console.log('Total TxInsValue !== total TxOutsValue in transaction: ' + transaction.id);
    return false;
  }
...
const validateTxIn = (txIn: TxIn, transaction: Transaction, unspentTxOuts: UnspentTxOut[]):boolean => {
  const referencedUTxOut: UnspentTxOut = 
    unspentTxOuts.find(uTxOut => uTxOut.txOutId === txIn.txOutId && uTxOut.txOutIndex === txIn.txOutIndex);
...
const isValidTransactionStructure = (transaction: Transaction): boolean => {
  ...
  if (!transaction.txIns.map(isValidTxInStructure).reduce((f, s) => (f && s), true)) {
    return false;
  }
